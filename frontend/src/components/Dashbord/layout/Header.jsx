...
import { SearchOutlined, StarOutlined, TwitterOutlined, FacebookFilled, UserOutlined, SettingOutlined, LogoutOutlined } from "@ant-design/icons";
...
const data = [
  { title: "New message from Sophie", description: <>{clockicon} 2 days ago</>, avatar: avtar },
  { title: "New album by Travis Scott", description: <>{clockicon} 2 days ago</>, avatar: <Avatar shape="square">{wifi}</Avatar> },
  { title: "Payment completed", description: <>{clockicon} 2 days ago</>, avatar: <Avatar shape="square">{credit}</Avatar> },
];

const menu = (
  <List min-width="100%" className="header-notifications-dropdown " itemLayout="horizontal" dataSource={data} renderItem={(item) => (
    <List.Item>
...
function Header({ placement, name, subName, onPress, handleSidenavColor, handleSidenavType, handleFixedNavbar }) {
  ...
  return (
    <>
      <div className="setting-drwer" onClick={showDrawer}>{setting}</div>
      <Row gutter={[24, 0]}>
        <Col span={24} md={6}>
          {/* Breadcrumb removed */}
        </Col>
        <Col span={24} md={18} className="header-control">
          <Badge size="small" count={4}>
            <Dropdown menu={{ items: menu }} trigger={["click"]}>
              <a href="#pablo" className="ant-dropdown-link" onClick={(e) => e.preventDefault()}>{bell}</a>
            </Dropdown>
          </Badge>
          ...
          <Drawer className="settings-drawer" mask={true} width={360} onClose={hideDrawer} placement={placement} open={visible}>
            <div layout="vertical">
              ...
              <div className="sidebar-color">
                ...
                <div className="theme-color mb-2" style={{ display: 'flex', gap: 8 }}>
                  <Button style={{ background: '#1890ff', color: '#fff', border: 'none' }} onClick={() => handleSidenavColor?.('#1890ff')}>1</Button>
                  <Button style={{ background: '#52c41a', color: '#fff', border: 'none' }} onClick={() => handleSidenavColor?.('#52c41a')}>1</Button>
                  <Button style={{ background: '#d9363e', color: '#fff', border: 'none' }} onClick={() => handleSidenavColor?.('#d9363e')}>1</Button>
                  <Button style={{ background: '#fadb14', color: '#111', border: 'none' }} onClick={() => handleSidenavColor?.('#fadb14')}>1</Button>
                  <Button style={{ background: '#111', color: '#fff', border: 'none' }} onClick={() => handleSidenavColor?.('#111')}>1</Button>
                </div>
                ...
                <div className="ant-docment">
                  <div style={{ display: 'flex', gap: 8 }}>
                    <Button size="large">FREE DOWNLOAD</Button>
          ...
          >
            <div className="user-profile-dropdown">
              ...
              <div className="user-info">
                <span className="user-name">{user?.firstName || user?.email || 'Utilisateur'}</span>
                <span className="user-role" style={{ color: getRoleColor(user?.role) }}>
                  {getRoleDisplayName(user?.role) || 'Utilisateur'}
                </span>
              </div>
            </div>
          </Dropdown>
